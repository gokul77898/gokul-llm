...
def _safe_load_causal(local_path, trust_remote_code=False, dtype=None):
    """
    Safe loader for causal models on CPU. Avoid load_in_4bit/bitsandbytes.
    dtype: torch.float16 or torch.float32 or None
    """
    map_args = {"device_map": "cpu"}
    load_kwargs = {}
    if dtype is not None:
        load_kwargs["torch_dtype"] = dtype
    # prefer local_path folder if exists
    if os.path.isdir(local_path):
        return AutoModelForCausalLM.from_pretrained(local_path, **load_kwargs, **map_args, trust_remote_code=trust_remote_code)
    # fallback: try HF id
    return AutoModelForCausalLM.from_pretrained(local_path, **load_kwargs, **map_args, trust_remote_code=trust_remote_code)
...
def load_expert_model(expert_name: str, device: Optional[str] = None):
    ...
    try:
        ...
        if ("llama" in expert.model_id.lower() or 
            "gpt" in expert.model_id.lower() or 
            "gemma" in expert.model_id.lower() or
            any(task in ["qa", "generation", "summarization", "reasoning"] for task in expert.task_types)):
            # Use safe loader for generative models (especially Gemma)
            dtype = torch.float16 if target_device == "cuda" else torch.float32
            model = _safe_load_causal(model_source, trust_remote_code=False, dtype=dtype)
        else:
